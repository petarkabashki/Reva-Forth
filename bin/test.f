...
: test>desc ( ix -- a n )
	...
	cell- @ count ;
| Bail-out:
:: ." Exception: " .x ." executing test #" number ? cr
...
: test
	...
	ms@ >r
	NUMBER-OF-TESTS @ 0do i do-or-die loop
	ms@ r> - 
	cr
...
exit~
macro
 : -TESTS- ~sys.srcstr eval ~test.test .used bye ;
forth
...
: mix
	#arr 10 + #arr do
		random abs #arr mod cells arr +
		random abs #arr mod	cells arr +					| x y -- two random array elements to exchange
		over @										| x y x@
		over @										| x y x@ y@
		-rot										| x y@ y x@
		swap !	swap !
