...
forth
 : xchg ( n a -- a^ ) [ $068bc389 , $5004768d , $038f038b , ;
...
: rot inline{ 89 C1 8B 46 04 8B 1E 89 5E 04 89 0E } ;
: -rot inline{ 8B 4E 04 89 46 04 8B 1E 89 0E 89 D8 } ;
...
| : dodoes dup inline{ 5b 58 53 } ; | dup; pop ebx; pop eax; push ebx
| : pdoes last@ >xt @ cell+ r> over - cell- swap !  ;
...
: pdoes 
	...
	r> lastxt cell- !
	;
: create parsews : (create) align 0 , (header) ['] 'create last@ cell- ! ;
macro
: does> ['] pdoes compile 
	here
	last @ >name
...
: (") dup 256 <if ['] dostr compile asciiz, ;then ['] dolstr compile asciizl, ;
...
: -2nip ( a b c d -- a d ) inline{ 83 c6 08 } ;inline
...
: loop  $240cff 3, | dec [esp] 
    ...
    again
    ;
: p[ repeat parsews 2dup " ]p" cmp 0if 2drop ;then (find) dup cell+ @ literal, cell- @ compile again ;
forth
| ========================= DICT ==================
variable findprev?
variable foundprev
...
: xt>name  ( xt -- a n )
	...
	xtdict @ ?dup if >name count else 0L then
    ;
...
: setclass ( class -- )
    ...
    default_class !
    ;
: .contexts { @ dup ctx>name type space } all-contexts iterate ;
: .~ { ctx>name type space } contexts stack-iterate ;
: ''context
...
: 'context compiling? if literal, p: ''context ;then ''context ;
: context: parsews
: (context) ( a n -- )
    (find) 0if
        (create) 
...
: context? xt? dup 3cell+ @ $54585443 <>if ." not a context" -1 throw then ; 
macro
...
: alias: ( newname oldname -- )
    ...
    ?dup 0if type ."  not found" cr -1 throw then
...
defer: see ' ?dup 0if type '? emit else
...
: [IF]
    ...
    drop srcstr drop 2dup -
    " [ELSE]" search if
        drop 2dup >if swap cell+ 1+ 1+ (else) !  then 
    then
    swap 0if cell+ 1+ 1+ >in !  else drop then
;
...
::
	2dup '. split if
		find dup if
			in~ ~sys find-word if -2nip 1 ;then
			2dup
		else
			3drop 
		then
...
~priv
with~ ~sys
:: 4096 allocate to scratch
    17 allocate to dump$ | space for 16 characters
    #env-var-buffer allocate to env-var-buffer
    256 allocate to dirbuf 
    ; onstartup
without~
exit~
: argv argc 1- clamp (argv) swap ?dup 0if zcount ;then 0do zcount + 1+ loop zcount ;
~reva
$06010a constant revaver# 
: revaver " 6.1.10" ;
...
::  " deinit-console" find ?dup if execute else 2drop then
    ." Exception: " .x ." at: " .x 100 ms ['] bye cr ; is exception
: hello 
    argc 1 do 
        i dup argv (hi) skip 
    loop ;
...
appdir pad place " reva" makeexename pad +place pad count (save) 
[THEN]
